...
void writeDataImp(void *param) {
  ...
  int code = taos_query(taos, "use db3");
  if (code != 0) {
    taos_error(taos);
  }
...
void writeData() {
  ...
  int code = taos_query(taos, "create database if not exists db3");
  if (code != 0) {
    taos_error(taos);
  }

  code = taos_query(taos, "create table if not exists db3.devices(ts timestamp, temperature int, humidity float) "
