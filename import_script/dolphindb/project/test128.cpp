...
static int rowsPerRequest=10000; //每次请求写入条数
static int totalRows=numOfFiles*1000000; //每个文件总行数
static string dataDir="/media/hj/aa/data125/dolphindb_testdata"; //测试数据路径
...
int main(int argc, char *argv[]){
    ...
    //get result
    string script_result="exec count(*) from loadTable('dfs://devDB131',`dev)";
    cout<<"exec count(*) from loadTable('dfs://devDB131',`dev):"<<endl;
...
void createDatabase(DBConnection conn)
{
    string script_schema="data=\"/media/hj/aa/data125/dolphindb_testdata/testdata0.csv\" \n schema = extractTextSchema(data) \n  update schema set name = \"devId\" where name= \"col0\" \n update schema set name = \"devName\" where name= \"col1\" \n update schema set name = \"devGroup\" where name= \"col2\" \n update schema set name = \"time\" where name= \"col3\" \n update schema set name = \"temperature\" where name= \"col4\" \n update schema set name = \"humidity\" where name= \"col5\" \ntypes= exec type from schema \n cols = exec name from schema \n  schema = table(50000:0, cols, types) \n";
    string script_createDatabase="dbDir = 'dfs://devDB131'\n if(existsDatabase(dbDir))\n dropDatabase(dbDir)\n range_schema=0..100 * 100 + 1\n db = database(dbDir, RANGE, range_schema)\n dev = db.createPartitionedTable(schema, `dev , `devId)\n";
    ...
    try{
        ...
        for(int i=pThread->sID;i<=pThread->eID;++i)
        {
            ...
            while (start < t->size())
            {
                ...
                conn.run("tableInsert{loadTable('dfs://devDB131',`dev)}", args); 
...
void readData(DBConnection conn, string queryScript)
{
    ...
    conn.run("dev=loadTable('dfs://devDB131', `dev)"); //load table
